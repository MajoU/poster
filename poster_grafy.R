...
data <- read.csv("./ph_fosfat.csv")
ph <- ggplot(data, aes(x = Time, y = ph))
pH <- ph + geom_line(colour="brown", size=1.8) + theme_bw() +
theme(text = element_text(size = 18), axis.text.y = element_text(size = 14, 
        colour = "darkblue"), axis.text.x = element_text(size = 14,colour = "darkblue"), 
          axis.title.y = element_text(vjust = 1.6), plot.margin=unit(c(1,3,1,1), "cm")) +
...
theme(text = element_text(size = 18), axis.text.y = element_text(size = 14, colour="darkblue"), 
axis.text.x = element_text(size = 14, colour="darkblue"), axis.title.y = element_text(vjust = 1.6), plot.margin=unit(c(1,1,1,3), "cm")) + 
labs(x = "Time [min]", y = "Phosphate (mM)" ) +
ggtitle("Phosphate concentration during ischemia")

pH.P <- arrangeGrob(pH, P, ncol = 2)    
ggsave("graf1.pdf", pH.P, width = 20, height = 6)
...
theme(text = element_text(size = 18), axis.text.y = element_text(size = 14, colour = "darkblue"), axis.text.x = element_text(size = 14,colour = "darkblue"), 
      axis.title.y = element_text(vjust = 1.6), plot.margin=unit(c(1,3,1,1), "cm")) +
...
ggsave("graf2.pdf", fos.ph, width = 20, height = 6)

#---------------------------------------
#  GRAF 3
#---------------------------------------

df <- read.csv("ischemia.csv")
...
theme(legend.position="bottom", legend.key.size = unit(0.8,"cm"), legend.background = element_rect(fill="grey90", size=0.5, linetype="dotted"),
      legend.key.width = unit(1.5,"cm"), axis.text.x = element_blank(),
      axis.text.y = element_text(size=15,color="darkblue"), text = element_text(size = 19), 
...
ggsave("graf3.pdf", gg, width = 13, height = 6)
